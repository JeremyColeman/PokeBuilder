    ...
    default ItemStack getItemStack(Player player, Pokemon pokemon) {
        return ItemStack.builder()
                ...
                .add(Keys.ITEM_LORE, Collections.singletonList(getCost(pokemon) == 0 ? Text.EMPTY : Text.of(
                        ...
                        getCost(pokemon) + " " + PokeBuilder.instance.currency.getPluralDisplayName().toPlain()
                )))
                .build();
    }

    default double getMultiplier(Pokemon pokemon) {
        return (Arrays.stream(EnumSpecies.LEGENDARY_ENUMS).anyMatch(p -> pokemon.getSpecies() == p) || pokemon.getSpecies() == EnumSpecies.Ditto ? PokeBuilder.instance.config.legendaryOrDittoMultiplier : 1);
    }
}
