...
import net.eterniamc.pokebuilder.ChatGuiHelper;
            ...
            if (pixelmon != null) {
                if (!Config.blacklistedPokemon.stream().anyMatch(s -> s.equalsIgnoreCase(pixelmon.getDisplayName()))) {
                    main.addElement(new ActionableElement(
                                    new RunnableAction(container, ActionType.NONE, "", c -> createEditorPage(container, pixelmon, player, Utils.getEconomy()).openState(player, "editor")),
                                    ItemStack.builder()
                            ...
                            if (cash < Config.pokemonCost && cash < Config.legendaryCost)
                                ...
                                ChatGuiHelper.addGUI("Enter the name of the pokemon you want, regular pokemon getCost(pokemon) $" + Config.pokemonCost + " and legendaries getCost(pokemon) $" + Config.legendaryCost, player, text -> {
                                    ...
                                    if (pokemon == null) {
                                        ...
                                        Utils.getEconomy().getOrCreateAccount(player.getUniqueId()).ifPresent(a -> a.withdraw(PokeBuilder.getCurrency(), new BigDecimal(Config.legendaryCost), Cause.of(EventContext.empty(), this)));
                                    } else {
                                        Utils.getEconomy().getOrCreateAccount(player.getUniqueId()).ifPresent(a -> a.withdraw(PokeBuilder.getCurrency(), new BigDecimal(Config.pokemonCost), Cause.of(EventContext.empty(), this)));
                                    }
